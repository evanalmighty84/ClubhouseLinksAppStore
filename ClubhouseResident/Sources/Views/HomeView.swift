...
struct ResidentProfileView: View {
    ...
    private var residentInitials: String {
    ...
    private var isAppleSignup: Bool {
        residentSignupProvider.lowercased() == "apple"
    }
    ...
    private var signupProviderBadge: some View {
        HStack(spacing: 4) {
            Image(systemName: isAppleSignup ? "apple.logo" : "envelope.fill")
            .font(.caption.bold())

            Text(isAppleSignup ? "Apple" : "Email")
            .font(.caption2.bold())
        }
        .foregroundStyle(isAppleSignup ? .white : .cyan)
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(isAppleSignup ? .black.opacity(0.55) : .cyan.opacity(0.15))
        .clipShape(Capsule())
    }
